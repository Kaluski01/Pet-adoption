...
export default function SingleDog() {
  ...
  useEffect(() => {
    const fetchDogData = () => {
      // Check if the name corresponds to a stored pet
      const storedPets = localStorage.getItem('pets');
      const existingPets = storedPets ? JSON.parse(storedPets) : [];
      const storedDog = existingPets.find((pet) => pet.name === name);
      ...
      // If not stored, fetch details from API
        ...
        <div className="row">
          <div className="col-lg-12 mb-5">
            {dog ? (
              ...
              <h1>Loading   <Spinner animation='grow' /></h1>
            )}
          </div>
        </div>
      </div>
